...
class Net(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=32, normalization='none', has_dropout=False, deep_supervision=False,max_n_filters=320):#,patch_size = [96,160,160],max_pool=6):
        ...
        self.block_en1_dw = DownsamplingConvBlock(n_filters, n_filters* 2, normalization=normalization)
        ...
        self.block_de2_up = UpsamplingDeconvBlock(n_filters * 2, n_filters, normalization=normalization)
    ...
    def decoder(self, features):
        ...
        if(self.deep_supervision == True):
            ...
            out1 =F.interpolate(self.out_conv1(x2d), scale_factor=2, mode='trilinear')
            out2 = F.interpolate(self.out_conv2(x3d), scale_factor=4, mode='trilinear')#nn.functional.interpolate(, scale_factor=4, mode='trilinear')
            out3 = F.interpolate(self.out_conv3(x4d), scale_factor=8, mode='trilinear')
